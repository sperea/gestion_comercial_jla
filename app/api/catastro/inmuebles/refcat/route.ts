  ...
  try {
    ...
    if (!refcat) {
      return NextResponse.json(
        { error: 'Falta parámetro requerido: refcat' },
        { status: 400 }
      )
    }
    ...
    console.log('🔑 Inmuebles por refcat - Cookies de autenticación:', {
    ...
    if (!accessToken) {
      console.log('❌ Inmuebles por refcat - No se encontró access-token')
      return NextResponse.json(
    ...
    const apiUrl = buildUrl('/catastro/inmuebles/refcat', params)

    console.log('📍 Referencia catastral:', refcat)
    console.log('🌐 Llamando a la API de inmuebles por refcat:', apiUrl)
    ...
    console.log('📋 Respuesta de la API de inmuebles por refcat:', {
    ...
    console.error('Error en la búsqueda de inmuebles por refcat:', error)
    return NextResponse.json(
      { 
        success: false, 
